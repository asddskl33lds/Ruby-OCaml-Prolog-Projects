...
class FiniteAutomaton
	...
	def to_dfa
		...
		dfa = FiniteAutomaton.new
		startState = dfa.new_state
		dfa.set_start startState
		...
		#Put the starting state in there
		sState.push @start
		
		#Ep.Closure of starting state
		sState = eClosure(sState,[])
		...
		while (myList.empty? == false)
			...
			next if visit[sState]
			visit[sState] = true
			dSource = dfaState[sState]
			
			@alphabet.each{ |x|  #get each alphabet		
				mov = []
				goingArray = []
				
				sState.each{ |v1|
					v2 = (get_transition(v1,x)) #move - gets an array
					if v2
						mov = mov|v2
					end
				}
				...
				next if (goingArray.empty?)
				
				sStatev2 = eClosure(goingArray,[])
				dv2 = dfaState[sStatev2]
				
				unless dv2
					dv2 = dfa.new_state
					dfaState[sStatev2] = dv2
					myList.push sStatev2
				end
				if dv2
					dfa.add_transition(dSource,dv2,x)
					
					if(dfa.alphabet.include?(x) == false)
						dfa.alphabet.push(x)
					end
				end
			}
...
=begin	
		dfa.new_stat
		dfa.push(eClosure([@start],[]))
		copyList.push(eClosure([@start][]))
		
		finalList.each{ |finalArray|   #Get first array of smallList
			finalArray.each{ |ele|      #Get first element of array
				ecm = Array.new
				mov = Array.new
				@alphabet.each{ |x|  #get each alphabet			
					mov =(get_transition(ele,x)) #move - gets an array
					mov.each {|mo|
						ecm = ecm|(eClosure[mo][])
					}
				copyList.each{ |getCopyArray|
					getCopyArray = getCopyArray.sort
					ecm = ecm.sort
					if (getCopyArray.eql?(ecm) == false)
						finalList.push(ecm)
						dfa.add_transition 
					end
				}
